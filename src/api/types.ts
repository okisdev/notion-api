...
type SubDecorationType = BoldFormatType | ItalicFormatType | StrikeFormatType | CodeFormatType | LinkFormatType | DateFormatType | UserFormatType | PageFormatType;
...
export type ColumnType = 'select' | 'text' | 'date' | 'person' | 'checkbox' | 'title' | 'multi_select' | 'number' | 'relation' | 'file' | 'email' | 'phone_number' | 'url';
...
export type RowContentType = string | boolean | number | string[] | { title: string; id: string } | UserType[] | DecorationType[] | { name: string; url: string }[];
