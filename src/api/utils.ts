...
export const idToUuid = (path: string) => `${path.substr(0, 8)}-${path.substr(8, 4)}-${path.substr(12, 4)}-${path.substr(16, 4)}-${path.substr(20)}`;
...
export const getNotionValue = (val: DecorationType[], type: ColumnType, row: RowType): RowContentType => {
    switch (type) {
        case 'text':
        ...
        case 'relation':
            return val.filter(([symbol]) => symbol === '‣').map(([_, relation]) => relation![0][1] as string);
        case 'file':
            return val
                .filter((v) => v.length > 1)
                .map((v) => {
